...
int main(int argc, char **argv) {
    ...
    for (; ; ) {
        ...
        if (cnt == -1) {
            fprintf(stderr, "Error %d in poll()", errno);
            perror("Error in poll()");
            exit(1);
        } else if (cnt == 0) {
            printf("poll timeout occurred for timeout of %ds\n", poll_timeout/1000);
            continue;
        }
         ...
         * for each socket which is returned by poll, read the data and respond back
         */
        ...
        /* remove all the fds which are closed */
        for (ii = 1; ii <= max_ifd; ii++ ) {
          if (client[ii].fd == -1) {
             client[ii].fd = client[max_ifd].fd;
             client[ii].events = client[max_ifd].events;
             client[ii].revents = client[max_ifd].revents;
             max_ifd--;
          }
       }
    }
